...
class InspeccionSitioScreen extends StatefulWidget {
  const InspeccionSitioScreen({super.key});

  @override
  State<InspeccionSitioScreen> createState() => _InspeccionSitioScreenState();
}

class _InspeccionSitioScreenState extends State<InspeccionSitioScreen> {
  final AjustadorApi api = AjustadorApi();

  late Future<List<SiniestroParte>> futureInspecciones;
  late Future<List<Siniestro>> futureSiniestros;

  @override
  void initState() {
    super.initState();
    futureInspecciones = api.getInspecciones();
    futureSiniestros = api.getSiniestros();
  }

  Future<void> _recargarInspecciones() async {
    setState(() {
      futureInspecciones = api.getInspecciones();
      futureSiniestros = api.getSiniestros();
    });
  ...
  String _normalizarEstado(String? estado) {
    ...
    switch (e) {
      ...
      case 'finalizada':
        return 'Completada';
      default:
        return estado?.trim().isNotEmpty == true ? estado!.trim() : 'Pendiente';
    }
  }
  ...
  Future<void> _crearInspeccion() async {
    ...
    final climaCtrl = TextEditingController();
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, setModalState) {
          ...
          return AlertDialog(
            title: const Text('Nueva inspección en sitio'),
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Condiciones del lugar',
                      border: OutlineInputBorder(),
                    ),
                  ),
                  const SizedBox(height: 12),
                  ...
                  TextField(
                    ...
                    maxLines: 3,
                    decoration: const InputDecoration(
                      labelText: 'Descripción de hallazgos',
                      border: OutlineInputBorder(),
                    ),
                  ),
                  const SizedBox(height: 12),
    ...
    final condiciones = condicionesCtrl.text.trim();
    final clima = climaCtrl.text.trim();
    ...
    if (siniestroSeleccionadoId == null || lugar.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Selecciona un siniestro y escribe el lugar'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      await _recargarInspecciones();

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Inspección registrada')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al registrar inspección: $e')),
      );
    }
  }

  Future<void> _eliminarInspeccion(SiniestroParte item) async {
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _datoDetalle(
                    'Lugar',
                    _textoContenido(item, 'lugar', fallback: item.titulo),
                  ),
                  _datoDetalle(
                    'Fecha',
                    item.fechaEvento ?? '—',
                  ...
                  _datoDetalle(
                    'Condiciones',
                    _textoContenido(item, 'condiciones'),
                  ),
                  _datoDetalle(
                    'Clima',
                    _textoContenido(item, 'clima'),
                  ),
                  _datoDetalle(
                    'Estado',
                    estado,
                  ...
                  const SizedBox(height: 12),
                  _bloqueTexto(
                    titulo: 'Descripción de hallazgos',
                  ...
                  OutlinedButton.icon(
                    onPressed: () {
                      ...
                      _eliminarInspeccion(item);
                    },
                    icon: const Icon(Icons.delete_outline),
                    label: const Text('Eliminar inspección'),
                  ),
                ],
              ),
            ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            'Cuando registres inspecciones en sitio aparecerán aquí.',
            style: TextStyle(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(
            onPressed: _recargarInspecciones,
  ...
  Widget _listaInspecciones(List<SiniestroParte> inspecciones, Color accent) {
    if (inspecciones.isEmpty) {
      return _emptyState(accent);
    }

    return Column(
      children: inspecciones.map((item) {
        final estado = _normalizarEstado(
          item.contenidoJson['estado']?.toString() ?? item.estado,
        );

        final colorEstado = _colorEstado(estado);
        final lugar = _textoContenido(item, 'lugar', fallback: item.titulo);
        final clima = _textoContenido(item, 'clima');
        final condiciones = _textoContenido(item, 'condiciones');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Inspección en sitio'),
        actions: [
          IconButton(
            tooltip: 'Recargar',
            onPressed: _recargarInspecciones,
        ...
        children: [
          ...
          SafeArea(
            child: RefreshIndicator(
              onRefresh: _recargarInspecciones,
              child: SingleChildScrollView(
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Container(
                      ...
                      child: FutureBuilder<List<SiniestroParte>>(
                        future: futureInspecciones,
                        builder: (context, snapshot) {
                          final inspecciones = snapshot.data ?? [];

                          final completadas = inspecciones.where((item) {
                            final estado = _normalizarEstado(
                          ...
                          return Column(
                            ...
                            children: [
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          Text(
                                            '${_saludoPorHora()}, inspección en sitio',
                                            style: const TextStyle(
                                              color: Colors.white,
                                              fontWeight: FontWeight.w900,
                                              fontSize: 16,
                                          ...
                                          const SizedBox(height: 4),
                                          Text(
                                            'Registro de condiciones, hallazgos y observaciones del lugar.',
                                            style: TextStyle(
                              ...
                              if (snapshot.connectionState ==
                                ...
                                _listaInspecciones(inspecciones, accent),
                            ],
                          );
                        },
                      ),
